...
| GDELT media/event metadata | GDELT data page states that the entire GDELT database is free and open. | Candidate transformed static event/crowd/weather witness; no live runtime fetch and no copied article bodies. | Usable for authored event-weather and crowd-pressure fixtures when exact query, retrieval date, fields, and transformation are recorded. |
| Wikimedia Pageviews | Wikimedia Analytics Pageviews download page states that all Analytics datasets are under the Creative Commons CC0 dedication. | Candidate transformed static public-attention witness; no live runtime fetch required. | Usable for attention/weather clues; pageviews are not sentiment, truth, causality, or trading evidence. |
| Coin Metrics Community Data | Coin Metrics docs state Community API data is free for non-commercial use under Creative Commons terms. | Candidate transformed network/exit-friction witness only; not currently bundled. | Usable only for non-commercial educational fixtures unless prize/reward/commercial use is reviewed or separate permission is obtained. |
| NimiRun V2 asset pack | Project-owner-provided generated asset pack, copied from the local source path listed below. | Bundled runtime art under `public/assets/nimirun-v2/`, including node-map PNGs and sanitized V2 decision-scene SVG placeholders. | No stock/source pack is known; if the submission form asks for generation tool/model proof, project owner should provide that provenance note. |
...
| CRPM repo: `C:\Users\jensb\Desktop\Projects\CRPM` | Design discipline / conceptual source | Provides the internal discipline behind pressure, cuts, carriers, residue, protected outcomes, landfall, and re-entry. Pocket Bot translates these into player-facing game terms. The current market-world lineage binding cites specific CRPM voyage/source-pressure anchors in `docs/product/market_world_model.md`. | Not a software dependency; not bundled; normal UI should not expose raw CRPM jargon. |
...
| NimiRun V2 game asset pack: `C:\Users\jensb\Downloads\nimirun_v2_game_assets` | Project-owner-provided generated runtime asset pack | Provides the current PB-005 runtime art: source-ocean map background, node pads/icons/rings, path threads, fog overlays, Pocket Bot V2 sprites, context slot images, HUD/proposal/trace panel frames, and the V2 phone decision-scene placeholders for arena card, signal glow, support/exit/crowd surfaces, narrator sigil, action buttons, and trace drawer. | Bundled into the app under `public/assets/nimirun-v2/`; manifest and preload helper live under `src/game/assets/`. Decision-scene SVG placeholders are sanitized so runtime labels remain dynamic. Generation tool/model and final competition license note should be confirmed before submission. |
...
| Market world model: `docs/product/market_world_model.md`, `src/game/scenarios/marketWorldLevels.js`, `src/game/scenarios/marketWorldLevelAdapter.js`, `src/domain/marketWorldRuntime.js`, and `src/ui/tracePanel.js` | Internal implementation-base design note / scenario contract scaffold | Converts the existing Market Signal Scout witness ledger, Binance fixture, historic headline witnesses, narrator role cut, and reward boundary into a relational market-arena contract. PB-014 adds an adapter that turns the Golden Signal market-world level into the live `arenaSpine`, relation-state seed, initial proposal preview, runtime-only hindsight card, and hindsight-free proposal context used by `marketSignalScoutScenario.js`; the domain runtime mutates relation status as arena actions are named, prepared, and approved, then derives false, partial, safe, or open finish packets from signal-support, signal-exit, and signal-crowd relation state. | Design note and source modules are in the repo and validated by tests. The current Phaser scene still renders through `marketSignalScoutScenario.js`, but its Golden Signal arena spine/proposal seed, relation-state mutation, relation-derived finish judgment, and post-finish hindsight-card display now come from the market-world adapter/runtime/UI path. This does not authorize live market data, investment advice, exchange execution, wallet authority, or persistent trading strategy export. |
| Pocket Bot arena narrator role cut: `docs/product/pocket_bot_arena_narrator_role_cut.md` | Internal CRPM-derived design lineage and role-boundary note | Defines the former signal-bot story, private narrator/world grammar, trace-backed repair rules, and player-facing language boundaries for Market Signal Scout. It explicitly names the CRPM repository artifacts that informed the design reading. | Documentation source only; not bundled as runtime code; its plain-language narrator copy informs Arena Spine V1. Normal UI should keep CRPM and relational-navigation terminology hidden behind game language. |
...
| GDELT Project data page: https://www.gdeltproject.org/data.html | Candidate world-layer witness source for event weather and crowd-pressure metadata | GDELT states that the entire database is free and open and can be downloaded, visualized, or analyzed through BigQuery. Pocket Bot may use bounded GDELT metadata as a transformed static witness for event volume, media attention, source URLs, themes, tone, and time-window pressure around a historic Bitcoin market window. | Candidate source; not yet bundled. Use only transformed metadata needed by the level. Do not copy article bodies, images, or broad raw exports into the app. Each adopted fixture must record query terms, covered dates, retrieved fields, retrieval date, transformation method, and residue that GDELT is a media-index witness, not source truth or trading evidence. |
| Wikimedia Analytics Pageviews API and dumps: https://doc.wikimedia.org/generated-data-platform/aqs/analytics-api/ and https://dumps.wikimedia.org/other/pageviews/readme.html | Candidate world-layer witness source for public-attention pressure | Wikimedia documents open Analytics API access to pageview metrics, and the Pageviews dataset page states that all Analytics datasets are under the Creative Commons CC0 dedication. Pocket Bot may use transformed pageview counts for relevant pages such as Bitcoin, Bitcoin futures, or named exchanges as a static attention/weather witness. | Candidate source; not yet bundled. CC0 makes the data suitable for transformed static game fixtures, but it must be scoped as attention volume only. It does not establish sentiment, causality, market truth, or investment advice. |
| Coin Metrics Community Data / Community API: https://gitbook-docs.coinmetrics.io/packages/coin-metrics-community-data and https://gitbook-docs.coinmetrics.io/access-our-data/api | Candidate world-layer witness source for network/exit-friction pressure | Coin Metrics states Community Data is available under a Creative Commons license and that Community API data is free for non-commercial use without an API key. Pocket Bot may use it as an authoring source for non-commercial educational network/exit-friction clues such as congestion or fee pressure, if exact metrics and query windows are recorded. | Candidate source with restriction; not yet bundled. Because the community license is non-commercial, do not use Coin Metrics data in reward-bearing, monetized, or ambiguous commercial submission material without review or separate permission. Prefer transformed static fixture values, not live API calls. |
