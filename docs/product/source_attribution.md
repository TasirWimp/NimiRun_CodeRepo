...
| Nimiq local Mini App and testnet guide: https://nimiq.dev/mini-apps/load-local-mini-app | Development and manual-test documentation | Defines the local Mini App loading path and testnet-switch behavior used for PB-012 manual test planning. The current implementation still needs device/emulator verification before competition readiness can pass. | Development/manual-test source only; not bundled. |
