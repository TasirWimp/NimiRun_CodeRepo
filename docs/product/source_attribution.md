...
| Market world model: `docs/product/market_world_model.md`, `src/game/scenarios/marketWorldLevels.js`, and `src/game/scenarios/marketWorldLevelAdapter.js` | Internal implementation-base design note / scenario contract scaffold | Converts the existing Market Signal Scout witness ledger, Binance fixture, historic headline witnesses, narrator role cut, and reward boundary into a relational market-arena contract. PB-014 adds an adapter that turns the Golden Signal market-world level into the live `arenaSpine`, relation-state seed, initial proposal preview, and hindsight-free proposal context used by `marketSignalScoutScenario.js`. | Design note and source modules are in the repo and validated by tests. The current Phaser scene still renders through `marketSignalScoutScenario.js`, but its Golden Signal arena spine/proposal seed is now sourced from the market-world adapter. This does not authorize live market data, investment advice, exchange execution, wallet authority, or persistent trading strategy export. |
